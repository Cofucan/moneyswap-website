      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-center">Salutation'</th>
                    ...
                    <th class="text-center">haslogin </th>
        ...
        <tbody>
            ...
            <tr class="person{{$person->id}}">
                ...
                <td>{{$person->gender}}</td>
                <td>{{$person->avatar}}</td>
                <td>{{$person->haslogin}}</td>
                
                <td><button class="edit-modal btn btn-info"
                        data-info="{{$person->id}},{{$person->first_name}},{{$person->last_name}},{{$person->population_estimate}},{{$person->state_id}},{{$person->longitude}},{{$person->latitude}}">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" role="form">
						...
						<div class="form-group">
							<label class="control-label col-sm-2" for="person_code">person Code</label>
							<div class="col-sm-10">
								<input type="name" class="form-control" id="person_code">
							</div>
						</div>
						<p class="fname_error error text-center alert alert-danger hidden"></p>
						<div class="form-group">
							<label class="control-label col-sm-2" for="person_name">Last Name:</label>
							<div class="col-sm-10">
								<input type="name" class="form-control" id="person_name">
							</div>
						</div>
...
function fillmodalData(details){
    $('#fid').val(details[0]);
    $('#person_code').val(details[1]);
    $('#person_name').val(details[2]);
    ...
    $('.modal-footer').on('click', '.edit', function() {
        $.ajax({
            ...
            success: function(data) {
            	if (data.errors){
                	$('#myModal').modal('show');
                    if(data.errors.person_code) {
                    	$('.fname_error').removeClass('hidden');
                        $('.fname_error').text("First name can't be empty !");
                    }
                    if(data.errors.person_name) {
