...
<div class="row">
  ...
  <div class="col-md-2">
	  <div class="page_button">
	 	<a href="{{ url('neighbourhoods/upload') }}"><button class="btn btn-sm btn-primary">Import  <i class="fa fa-arrow-down"></i></button></a>
      </div>
        {{-- modal begins--}}
                            ...
                            {{-- modal begins--}}
                                <div class="modal fade bd-example-modal-lg{{$neighbourhood->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabelS" aria-hidden="true">
                                    <div class="modal-dialog modal-md modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            <h5 class="modal-title text-center">Edit {{$neighbourhood->neighbourhood_name}}</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                            </div>
                                            <div class="modal-body">
                                                ...
                                                <form method="POST" action="{{ route('neighbourhoods.update', $neighbourhood->id) }}" id="UpdateDesignation" enctype="multipart/form-data"> 
                                                    {{csrf_field()}}  
                                                    @method('PUT')
                                                   <input type="hidden" name="neighbourhood_id" id="neighbourhood_id" value="{{$neighbourhood->id}}">             
                                                   <input type="hidden" name="city_id" id="city_id" value="{{$neighbourhood->city_id}}">             
