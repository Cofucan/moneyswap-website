...
class MessagesController extends Controller
{
    ...
    {
        if(isset($this->data['message_id']) || isset($this->message_id)){
            $this->message = Message::findorFail(!empty($this->data['message_id']) ? $this->data['message_id'] : $this->message_id);

        }else{
            $this->message = new Message;
        }
            $this->message->message_subject = $this->data['message_subject'];
            $this->message->message_body = $this->data['message_body'];
            $this->message->status = !empty($this->data['status']) ? $this->data['status'] : 'Draft';
            $this->message->parent_id = !empty($this->data['parent_id']) ? $this->data['parent_id'] : '0';
            if ( ! $this->user->Messages()->save($this->message)) {
    ...
    public function mailbox($action){
        $messages = Message::where('status', $action)->where('user_id', Auth::User()->id)->get();
    ...
    public function destroy(Message $message)
    {
        //
        $message->delete();
         return redirect()->route('messages.manage')
