...
<div class="modal fade" id="new" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabelS" aria-hidden="true">
    <div class="modal-dialog modal-md modal-dialog-centered">
        <div class="modal-content">
            ...
            <h5 class="modal-title text-center">Add Price Category</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
            </div>
            <div class="modal-body">
                <form method="POST" action="{{ route('pricecategories.store') }}" id="CreatePriceCategory">
                {{csrf_field()}}

                <div class="form-group">
                    <label for="label"> Category </label>
                    <input type="text" name="label" value="{{old('label')}}" class="form-control" placeholder="Enter category name"  id="label" />
                    @if ($errors->has('label'))
