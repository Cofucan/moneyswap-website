...
<div class="modal fade" id="addfee{{ $price->id }}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabelS" aria-hidden="true">
    <div class="modal-dialog modal-md modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-center">Add Payment Price</h5>
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('fees.store') }}" id="CreateFee" >
                    ...
                    <div class="form-group mb-2">
                        <label for="price_id"> <strong> Price Name:</strong> {{  $price->label }}</label>
                        <input type="hidden" name="price_id" value="{{ $price->id }}" id="price_id" />
                        <input type="hidden" name="item_id" value="{{ $price->id }}" />
                    </div>
                    <div class="form-group">

                        <div class="custom-control custom-radio custom-control-inline">
                          <input id="general{{ $price->id }}" name="feeable_type" type="radio" value="" class="custom-control-input" required>
                          <label class="custom-control-label" for="general{{ $price->id }}">General Fee</label>
                        </div>
                        @if ($price->label == 'Transport')
                        <div class="custom-control custom-radio custom-control-inline">
                            <input id="Transport{{ $price->id }}" name="feeable_type" type="radio" value="fare" class="custom-control-input" required>
                            <label class="custom-control-label" for="Transport{{ $price->id }}">Transport</label>
                            <input type="hidden" name="city_id" value="1" id="city_id" />
                        </div>
                        @else
                        <div class="custom-control custom-radio custom-control-inline">
                            <input id="rate{{ $price->id }}" name="feeable_type" type="radio" value="program" class="custom-control-input" required>
                            <label class="custom-control-label" for="rate{{ $price->id }}">Apply to Program</label>
                        </div>
                        <div class="custom-control custom-radio custom-control-inline">
                            <input id="Level{{ $price->id }}" name="feeable_type" type="radio" value="level" class="custom-control-input" required>
                            <label class="custom-control-label" for="Level{{ $price->id }}">Apply to Class </label>
                    ...
                    <div id="showgrade" class="myDiv">
                        ...
                        <div class="mb-3 form-group">
                            <label for="label"> Name <span class="required"> (If empty, payment price name will be used)</span> </label>
