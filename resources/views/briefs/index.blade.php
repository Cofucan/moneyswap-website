@extends('layouts.admin')
@push('styles')
<link rel="stylesheet" href="{{ asset ('css/board.css') }}">
<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/v/bs4/dt-1.10.18/fh-3.1.4/r-2.2.2/datatables.min.css"/>
@endpush
@section('content')
        <div class="bread-crumb bg5 flex-w p-l-5 p-t-5 p-b-5 p-l-15-sm m-b-10">
            <a href="{{ url ('home')}}" class="s-text16">
                <i class="fa fa-home"></i> Dashboard
                <i class="fa fa-angle-right m-l-8 m-r-9" aria-hidden="true"></i>
            </a>

            <span class="s-text16">
              Briefs
                <i class="fa fa-angle-right m-l-8 m-r-9" aria-hidden="true"></i>
            </span>

            <span class="s-text17">
                Manage
            </span>
        </div>
<div class="row">
  <div class="col-md-9 content_title">
     	<h3> Briefs </h3>
	</div>

</div>
    <div class="row ">
      <div class="col-md-12 col-sm-12 student-info ">
            <!-- <div class="table-responsive"> -->

            <table class="table w-100 " id="table">

                    <tbody>
                      @foreach ($briefs as $day => $brief_list)
                      <thead>
                        <th colspan="2" style="background-color: #F7F7F7">{{ $day }}: {{ $brief_list->count() }} Briefs</th>

                        <th >Property Type</th>
                        <th >Location</th>
                        <th >Contract Type</th>
                        <th >Budget</th>
                        <th >Contact Name</th>
                        <th >Telephone</th>
                        <th >Email</th>
                        <th  width="20%">Actions</th>
                        </thead>
                      @foreach($brief_list as $brief)
                      <tr>
                            <td>{{$brief->id}}</td>                            
                            <td>{{ $brief->brief_subject }}</td>
                            <td>{{ $brief->RealtyType->realty_type }}</td>
                            <td>{{ $brief->Neighbourhood->neighbourhood_name }}</td>
                            <td>{{ $brief->contract_type }}</td> 
                             <td>{{ $brief->currency }} {{ number_format($brief->budget,2) }}</td>                        
                            <td>{{$brief->contact_name}}</td>
                            <td>{{$brief->telephone}}</td>
                            <td>{{$brief->email }}</td>
                            <td>
                                <div class="row no-gutters">
                                    <div class="col-md-9">
                                      <a class="btn btn-secondary btn-sm" href="{{ route('briefs.show', $brief->id) }}"><i class="fa fa-eye"></i></a>
                                      <a class="btn btn-primary btn-sm" href="{{ route('briefs.edit',$brief->id) }}"><i class="fa fa-edit"></i> </a>
                                    </div>
                                    <div class="col-md-2">
                                        <form action="{{ route('briefs.destroy',$brief->id) }}" method="post"
                                            onsubmit="return confirm('Are you sure you want to delete this record?');">
                                            <input type="hidden" name="_method" value="DELETE" />
                                            {{ csrf_field() }}
                                            <button type="submit" name="Delete" class="btn btn-sm btn-danger action_btn"> <i class="fa fa-trash"></i></button>
                                        </form>
                                    </div>
                                </div>
                      </tr>
                      @endforeach
                      @endforeach
                    </tbody>
                </table>
<!-- </div> -->
</div>
</div>

@endsection
@push('scripts')
<script type="text/javascript" src="https://cdn.datatables.net/v/bs4/dt-1.10.18/fh-3.1.4/r-2.2.2/datatables.min.js"></script>
<script>
  $(document).ready(function() {
    $('#table').DataTable();
} );
 </script>

 @endpush
